...
#else
#define STM32_DMA_ISR_MASK          0x7F00U
#endif
 ...
 */
#if defined(STM32U5) // STM32U5 PORT
#define STM32_DMA_IS_VALID_ID(id, mask) 1
 ...
 */
#if defined(STM32U5)  // STM32U5 PORT
#define STM32_DMA_CR_RESET_VALUE    (DMA_CCR_SUSP | DMA_CCR_RESET)
#else
 ...
 */
#if defined(STM32U5)  // STM32U5 PORT
#define STM32_DMA_CR_DMEIE (DMA_CCR_HTIE  | DMA_CCR_DTEIE | DMA_CCR_ULEIE | DMA_CCR_USEIE | DMA_CCR_TOIE)
#else
...
#if !defined(STM32U5) && (STM32_DMA_SUPPORTS_DMAMUX == FALSE) || defined(__DOXYGEN__) // STM32U5 PORT
 ...
 */
#if defined(STM32U5)  // STM32U5 PORT
#define STM32_DMA_FCR_RESET_VALUE   0x00007f00U
#else
...
typedef struct {
#if defined(STM32U5) // STM32U5 PORT
  DMA_Channel_TypeDef    *stream;        /**< @brief Associated DMA stream.  */
#else
...
#else
#define dmaStreamSetMemory0(dmastp, addr) {                                 \
  (dmastp)->stream->CDAR  = (uint32_t)(addr);                               \
}
#endif
...
#else
#define dmaStreamSetTransactionSize(dmastp, size) {                         \
  (dmastp)->stream->CBR1  = (uint32_t)(size);                               \
}
#endif
...
#else
#define dmaStreamGetTransactionSize(dmastp) ((size_t)((dmastp)->stream->CBR1))
#endif
...
#else
#define dmaStreamSetMode(dmastp, mode) {                                    \
  (dmastp)->stream->CTR2  = (uint32_t)(mode);                                 \
}
#endif
...
#else
#define dmaStreamDisable(dmastp) {                             	\
  (dmastp)->stream->CCR &= ~(STM32_DMA_ISR_MASK);      			\
	if (((dmastp)->stream->CCR & DMA_CCR_EN)) {					\
		(dmastp)->stream->CCR |= DMA_CCR_SUSP;                 	\
		while (!((dmastp)->stream->CCR & DMA_CCR_SUSP) &&		\
				(((dmastp)->stream->CCR & DMA_CCR_EN)));       	\
		(dmastp)->stream->CCR |= DMA_CCR_RESET;          		\
		while (((dmastp)->stream->CCR & DMA_CCR_EN))  ;       	\
	}					\
  dmaStreamClearInterrupt(dmastp);                             	\
}
#endif
...
#else
#define dmaStreamClearInterrupt(dmastp) {                        \
  (dmastp)->stream->CFCR = STM32_DMA_ISR_MASK ;                  \
}
#endif
