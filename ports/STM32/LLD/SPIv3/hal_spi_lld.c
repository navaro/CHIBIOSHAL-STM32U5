...
static uint32_t dummytx /*PLATFORM_SECTION_DMA*/  ;
static uint32_t dummyrx /*PLATFORM_SECTION_DMA*/ ;
...
void spi_lld_init(void) {
  ...
  dummytx = STM32_SPI_FILLER_PATTERN ;
...
void spi_lld_start(SPIDriver *spip) {
  ...
  if (spip->state == SPI_STOP) {
...
#else
      spip->rx.dma->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      spip->rx.dma->stream->CTR2 |= STM32_DMAMUX1_SPI1_RX ;
      spip->tx.dma->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      spip->tx.dma->stream->CTR2 |= STM32_DMAMUX1_SPI1_TX ;
#endif
    }
#endif
#if STM32_SPI_USE_SPI2
...
#else
      spip->rx.dma->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      spip->rx.dma->stream->CTR2 |= STM32_DMAMUX1_SPI2_RX ;
      spip->tx.dma->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      spip->tx.dma->stream->CTR2 |= STM32_DMAMUX1_SPI2_TX ;
#endif
    }
#endif
#if STM32_SPI_USE_SPI3
...
#else
      spip->rx.dma->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      spip->rx.dma->stream->CTR2 |= STM32_DMAMUX1_SPI3_RX ;
      spip->tx.dma->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      spip->tx.dma->stream->CTR2 |= STM32_DMAMUX1_SPI3_TX ;
#endif
    }
#endif
#if STM32_SPI_USE_SPI4
...
#else
      spip->rx.dma->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      spip->rx.dma->stream->CTR2 |= STM32_DMAMUX1_SPI4_RX ;
      spip->tx.dma->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      spip->tx.dma->stream->CTR2 |= STM32_DMAMUX1_SPI4_TX ;
#endif
    }
#endif
#if STM32_SPI_USE_SPI5
    ...
    #else
      spip->rx.dma->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      spip->rx.dma->stream->CTR2 |= STM32_DMAMUX1_SPI5_RX ;
      spip->tx.dma->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      spip->tx.dma->stream->CTR2 |= STM32_DMAMUX1_SPI5_TX ;
#endif
    }
#endif
#if STM32_SPI_USE_SPI6
...
#else
      spip->rx.dma->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      spip->rx.dma->stream->CTR2 |= STM32_DMAMUX1_SPI6_RX ;
      spip->tx.dma->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      spip->tx.dma->stream->CTR2 |= STM32_DMAMUX1_SPI6_TX ;
#endif
    }
#endif
...
#else
      spip->rx.dma->stream->CSAR = (uint32_t)&spip->spi->RXDR ;
      spip->tx.dma->stream->CDAR = (uint32_t)&spip->spi->TXDR ;
#endif
...
#endif
#if !defined(STM32U5) && defined(STM32_SPI_DMA_REQUIRED)
  {
    if (dsize <= 8U) {
      /* Frame width is between 4 and 8 bits.*/
...
void spi_lld_config(SPIDriver *spip) {
	  /* SPI setup and enable.*/
	  spip->spi->CR1 &= ~SPI_CR1_SPE;
	  spip->spi->CR1  = SPI_CR1_MASRX;
	  spip->spi->CR2  = 0U;
...
#else
    spip->rx.dma->stream->CSAR = (uint32_t)&spip->spi->RXDR ;
    spip->rx.dma->stream->CDAR = (uint32_t)rxbuf ;
    spip->rx.dma->stream->CBR1  = (uint32_t)(n);                               \
    spip->rx.dma->stream->CTR1  |= DMA_CTR1_DINC;
    spip->rx.dma->stream->CTR1  &= ~DMA_CTR1_SINC;
    spip->rx.dma->stream->CTR2  &= ~(DMA_CTR2_SWREQ|DMA_CTR2_DREQ);                               \
    spip->rx.dma->stream->CCR  |= DMA_CCR_TCIE|DMA_CCR_DTEIE|
...
#else
    spip->tx.dma->stream->CDAR = (uint32_t)&spip->spi->TXDR ;
    spip->tx.dma->stream->CSAR = (uint32_t)txbuf ;
    spip->tx.dma->stream->CBR1  = (uint32_t)(n);
    spip->tx.dma->stream->CTR1  |= DMA_CTR1_SINC;
    spip->tx.dma->stream->CTR1  &= ~DMA_CTR1_DINC;
    spip->tx.dma->stream->CTR2  &= ~(DMA_CTR2_SWREQ);
    spip->tx.dma->stream->CTR2  |= DMA_CTR2_DREQ;
    spip->tx.dma->stream->CCR  |= DMA_CCR_TCIE|DMA_CCR_DTEIE|
  		  	  	  	  	  	  	 DMA_CCR_ULEIE|DMA_CCR_USEIE;
#endif
...
#else
    spip->rx.dma->stream->CSAR = (uint32_t)&spip->spi->RXDR ;
    spip->rx.dma->stream->CDAR = (uint32_t)&dummyrx ;
    spip->rx.dma->stream->CBR1  = (uint32_t)(n);                               \
    spip->rx.dma->stream->CTR1  &= ~DMA_CTR1_DINC;
    spip->rx.dma->stream->CTR1  &= ~DMA_CTR1_SINC;
    spip->rx.dma->stream->CTR2  &= ~(DMA_CTR2_SWREQ|DMA_CTR2_DREQ);                               \
...
#else
    spip->tx.dma->stream->CDAR = (uint32_t)&spip->spi->TXDR ;
    spip->tx.dma->stream->CSAR = (uint32_t)&dummytx ;
    spip->tx.dma->stream->CBR1  = (uint32_t)(n);
    spip->tx.dma->stream->CTR1  &= ~DMA_CTR1_SINC;
    spip->tx.dma->stream->CTR1  &= ~DMA_CTR1_DINC;
    spip->tx.dma->stream->CTR2  &= ~(DMA_CTR2_SWREQ);
