...
#else
  (uartp->dmarx)->stream->CDAR  = (uint32_t)(&uartp->rxbuf);
  uartp->dmarx->stream->CTR1  &= ~DMA_CTR1_DINC;
  uartp->dmarx->stream->CTR2  &= ~(DMA_CTR2_SWREQ|DMA_CTR2_DREQ);  
  uartp->dmarx->stream->CBR1  |= 1; 
#endif
...
#define _tx1_isr_code(uartp) {                                         \
  ...
  if ((uartp)->threadtx) _uart_wakeup_tx1_isr(uartp);                                              \
}
// STM32U5xx port
#define _rx_complete_isr_code(uartp) {                                 \
  ...
  if ((uartp)->threadrx) _uart_wakeup_rx_complete_isr(uartp);              \
}
// STM32U5xx port
#define _tx2_isr_code(uartp) {                                         \
  (uartp)->txstate = UART_TX_COMPLETE;                                      \
  if ((uartp)->config->txend2_cb != NULL) {                                 \
    (uartp)->config->txend2_cb(uartp);                                      \
  }                                                                         \
  if ((uartp)->threadtx) _uart_wakeup_tx2_isr(uartp);                                              \
}
// STM32U5xx port
#define _rx_error_isr_code(uartp, errors) {                            \
  if ((uartp)->config->rxerr_cb != NULL) {                                  \
    (uartp)->config->rxerr_cb(uartp, errors);                               \
  }                                                                         \
  if ((uartp)->threadtx) _uart_wakeup_rx_error_isr(uartp);                                         \
}
/**
...
#else
  if ((flags & (DMA_CFCR_USEF | DMA_CFCR_ULEF | DMA_CFCR_DTEF)) != 0) {
#endif
...
#else
  UARTD2.dmarxmode = STM32_DMA_CR_DMEIE | DMA_CCR_TCIE;
  UARTD2.dmatxmode = STM32_DMA_CR_DMEIE | DMA_CCR_TCIE;
#endif
  UARTD2.dmarx   = NULL;
  UARTD2.dmatx   = NULL;
...
#else
  UARTD3.dmarxmode = STM32_DMA_CR_DMEIE | DMA_CCR_TCIE;
  UARTD3.dmatxmode = STM32_DMA_CR_DMEIE | DMA_CCR_TCIE;
#endif
  UARTD3.dmarx   = NULL;
  UARTD3.dmatx   = NULL;
...
#else
  UARTD5.dmarxmode = STM32_DMA_CR_DMEIE | DMA_CCR_TCIE;
  UARTD5.dmatxmode = STM32_DMA_CR_DMEIE | DMA_CCR_TCIE;
#endif
  UARTD5.dmarx   = NULL;
  UARTD5.dmatx   = NULL;
  ...
  if (uartp->state == UART_STOP) {
    ...
    if (false) {
      ...
      uartp->dmatxmode |= STM32_DMA_CR_CHSEL(USART1_TX_DMA_CHANNEL) |
                          STM32_DMA_CR_PL(STM32_UART_USART1_DMA_PRIORITY);
#if !defined(STM32U5) && STM32_DMA_SUPPORTS_DMAMUX
      dmaSetRequestSource(uartp->dmarx, STM32_DMAMUX1_USART1_RX);
      dmaSetRequestSource(uartp->dmatx, STM32_DMAMUX1_USART1_TX);
#elif defined(STM32U5)
      uartp->dmarx->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      uartp->dmarx->stream->CTR2 |= STM32_DMAMUX1_USART1_RX ;
      uartp->dmatx->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      uartp->dmatx->stream->CTR2 |= STM32_DMAMUX1_USART1_TX ;
#endif
    }
#endif
      ...
      uartp->dmarx = dmaStreamAllocI(STM32_UART_USART2_RX_DMA_STREAM,
                                     STM32_UART_USART2_IRQ_PRIORITY+2,
                                     (stm32_dmaisr_t)uart_lld_serve_rx_end_irq,
                                     (void *)uartp);
      osalDbgAssert(uartp->dmarx != NULL, "unable to allocate stream");
      uartp->dmatx = dmaStreamAllocI(STM32_UART_USART2_TX_DMA_STREAM,
                                     STM32_UART_USART2_IRQ_PRIORITY+1,
                                     (stm32_dmaisr_t)uart_lld_serve_tx_end_irq,
                                     (void *)uartp);
      osalDbgAssert(uartp->dmatx != NULL, "unable to allocate stream");
...
#elif defined(STM32U5)
      uartp->dmarx->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      uartp->dmarx->stream->CTR2 |= STM32_DMAMUX1_USART2_RX ;
      uartp->dmatx->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      uartp->dmatx->stream->CTR2 |= STM32_DMAMUX1_USART2_TX ;
#endif
    }
#endif
      ...
      uartp->dmarx = dmaStreamAllocI(STM32_UART_USART3_RX_DMA_STREAM,
                                     STM32_UART_USART3_IRQ_PRIORITY+2,
                                     (stm32_dmaisr_t)uart_lld_serve_rx_end_irq,
                                     (void *)uartp);
      osalDbgAssert(uartp->dmarx != NULL, "unable to allocate stream");
      uartp->dmatx = dmaStreamAllocI(STM32_UART_USART3_TX_DMA_STREAM,
                                     STM32_UART_USART3_IRQ_PRIORITY+1,
                                     (stm32_dmaisr_t)uart_lld_serve_tx_end_irq,
                                     (void *)uartp);
      osalDbgAssert(uartp->dmatx != NULL, "unable to allocate stream");
...
#elif defined(STM32U5)
      uartp->dmarx->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      uartp->dmarx->stream->CTR2 |= STM32_DMAMUX1_USART3_RX ;
      uartp->dmatx->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      uartp->dmatx->stream->CTR2 |= STM32_DMAMUX1_USART3_TX ;
#endif
    }
#endif
...
#elif defined(STM32U5)
      uartp->dmarx->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      uartp->dmarx->stream->CTR2 |= STM32_DMAMUX1_USART4_RX ;
      uartp->dmatx->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      uartp->dmatx->stream->CTR2 |= STM32_DMAMUX1_USART4_TX ;      
#endif
    }
#endif
      ...
      uartp->dmarx = dmaStreamAllocI(STM32_UART_UART5_RX_DMA_STREAM,
                                     STM32_UART_UART5_IRQ_PRIORITY+2,
                                     (stm32_dmaisr_t)uart_lld_serve_rx_end_irq,
                                     (void *)uartp);
      osalDbgAssert(uartp->dmarx != NULL, "unable to allocate stream");
      uartp->dmatx = dmaStreamAllocI(STM32_UART_UART5_TX_DMA_STREAM,
                                     STM32_UART_UART5_IRQ_PRIORITY+1,
                                     (stm32_dmaisr_t)uart_lld_serve_tx_end_irq,
                                     (void *)uartp);
      osalDbgAssert(uartp->dmatx != NULL, "unable to allocate stream");
...
#elif defined(STM32U5)
      uartp->dmarx->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      uartp->dmarx->stream->CTR2 |= STM32_DMAMUX1_UART5_RX ;
      uartp->dmatx->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      uartp->dmatx->stream->CTR2 |= STM32_DMAMUX1_UART5_TX ;
#endif
...
#elif defined(STM32U5)
      uartp->dmarx->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      uartp->dmarx->stream->CTR2 |= STM32_DMAMUX1_UART6_RX ;
      uartp->dmatx->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      uartp->dmatx->stream->CTR2 |= STM32_DMAMUX1_UART6_TX ;
#endif
    }
#endif
...
#elif defined(STM32U5)
      uartp->dmarx->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      uartp->dmarx->stream->CTR2 |= STM32_DMAMUX1_UART7_RX ;
      uartp->dmatx->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      uartp->dmatx->stream->CTR2 |= STM32_DMAMUX1_UART7_TX ;
#endif
    }
#endif
...
#elif defined(STM32U5)
      uartp->dmarx->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      uartp->dmarx->stream->CTR2 |= STM32_DMAMUX1_UART8_RX ;
      uartp->dmatx->stream->CTR2 &= ~DMA_CTR2_REQSEL_Msk ;
      uartp->dmatx->stream->CTR2 |= STM32_DMAMUX1_UART8_TX ;
#endif
    }
#endif
    else {
...
#else
    uartp->dmarx->stream->CSAR = (uint32_t)&uartp->usart->RDR ;
    uartp->dmatx->stream->CDAR = (uint32_t)&uartp->usart->TDR ;
#endif
    uartp->rxbuf = 0;
  }
...
#else
  uartp->dmatx->stream->CTR1 = (uartp->dmatx->stream->CTR1 | DMA_CTR1_SINC) &
		  	  	  	  	  	  ~DMA_CTR1_DINC ;
  uartp->dmatx->stream->CTR2 = (uartp->dmatx->stream->CTR2 | DMA_CTR2_DREQ) &
		  	  	  	  	  	  ~(DMA_CTR2_SWREQ);
  uartp->dmatx->stream->CCR  |= DMA_CCR_TCIE|DMA_CCR_DTEIE|
		  	  	  	   DMA_CCR_ULEIE|DMA_CCR_USEIE;
  uartp->dmatx->stream->CSAR = (uint32_t)txbuf ;
  uartp->dmatx->stream->CBR1  = (uint32_t)(n);
#endif
...
void uart_lld_start_receive(UARTDriver *uartp, size_t n, void *rxbuf) {
  ...
  dmaStreamSetMode(uartp->dmarx, uartp->dmarxmode  | STM32_DMA_CR_DIR_P2M |
                                 STM32_DMA_CR_MINC | STM32_DMA_CR_TCIE);
#else
  uartp->dmarx->stream->CTR1 = (uartp->dmarx->stream->CTR1 | DMA_CTR1_DINC) &
		  	  	  	  	  	  ~DMA_CTR1_SINC ;
  uartp->dmarx->stream->CTR2  &= ~(DMA_CTR2_SWREQ|DMA_CTR2_DREQ);
  uartp->dmarx->stream->CCR  |= DMA_CCR_TCIE|DMA_CCR_DTEIE|
		  	  	  	  DMA_CCR_ULEIE|DMA_CCR_USEIE;
  uartp->dmarx->stream->CDAR = (uint32_t)rxbuf ;
  uartp->dmarx->stream->CBR1  = (uint32_t)(n);
#endif
  /* Starting transfer.*/
  dmaStreamEnable(uartp->dmarx);
}
...
#else
    _rx_error_isr_code(uartp, translate_errors(isr));
#endif
  }
