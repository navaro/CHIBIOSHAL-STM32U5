 ...
 * @notapi
 */
#define pal_lld_maskpadevent(port, pad)                             \
  _pal_lld_maskpadevent(port, pad)
 ...
 * @notapi
 */
#define pal_lld_unmaskpadevent(port, pad)                                  \
  _pal_lld_unmaskpadevent(port, pad)
...
#define pal_lld_eventenable(port, pad)                                \
  EXTI->IMR1 |= (1U << (uint32_t)pad)

#define pal_lld_eventdisable(port, pad)                                \
  EXTI->IMR1 &= ~(1U << (uint32_t)pad)
...
extern "C" {
  ...
  void _pal_lld_maskpadevent(ioportid_t port, iopadid_t pad);
  void _pal_lld_unmaskpadevent(ioportid_t port, iopadid_t pad);
#ifdef __cplusplus
}
#endif
