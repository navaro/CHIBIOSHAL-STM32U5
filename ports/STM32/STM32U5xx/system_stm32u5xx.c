...
/** @addtogroup External declarations
  * @{
  */

extern void stm32_clock_init(void) ;
extern void stm32_board_init(void) ;
...
__attribute__( ( always_inline ) ) static inline unsigned long _tx_control_get(void)
{
...
__attribute__( ( always_inline ) ) static inline void _tx_control_set(unsigned long control_value)
{
...
void SystemInit(void)
{
  ...
  /* FPU settings ------------------------------------------------------------*/
   ...
   //unsigned long val = _tx_control_get() ;
   //_tx_control_set (val & ~(CONTROL_FPCA_Msk)) ;
#endif
  ...
  /* Configure the Vector Table location add offset address ------------------*/
    ...
    if ((controlReg & CONTROL_nPRIV_Msk) != 0) {
        // Switch to handler mode
        __set_CONTROL(controlReg & ~CONTROL_nPRIV_Msk);
        __DSB();  // Ensure the control register update is completed
        __ISB();  // Flush the pipeline to ensure the subsequent instructions are fetched in handler mode
    }
